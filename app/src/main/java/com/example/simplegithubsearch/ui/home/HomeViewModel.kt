...
class HomeViewModel(
  ...
  val isLoading = searchViewModel.doOnSubscribe.combineLatest(
    userList, object : Function2<Event<Unit>, List<UserDetail>, Boolean> {
      override fun apply(t1: Event<Unit>?, t2: List<UserDetail>?): Boolean {
  ...
  val userList: LiveData<List<UserDetail>> get() = searchViewModel.userList
}